...
export default function Users() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // If user is logged in, try to load their existing profile
          if (user) {
            ...
            if (userProfile) {
              setNewUserForm({
                ...
                role: userProfile.role || defaultRoles[0],
                department: userProfile.department || defaultDepartments[0],
  ...
  const create = async () => {
    ...
    try {
      ...
      // Create or update user profile for the logged-in user
      const profileData = {
        user_id: userId,
        full_name: newUserForm.firstName.trim(),
        email: newUserForm.email.trim(),
        mobile_number: newUserForm.mobileNumber || "",
      ...
      // Try to update existing profile first, if it doesn't exist, create new one
      let profileResponse;
      try {
        // Try PUT first (update existing profile)
        profileResponse = await fetch(`http://localhost:8000/user-profiles/${userId}`, {
          method: "PUT",
          headers: {
        ...
        // If update fails with 404, try POST (create new profile)
        if (!profileResponse.ok && profileResponse.status === 404) {
          profileResponse = await fetch("http://localhost:8000/user-profiles", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
        ...
        // If update fails, try POST (create new profile)
        profileResponse = await fetch("http://localhost:8000/user-profiles", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const profilesResponse = await fetch("http://localhost:8000/user-profiles");
      ...
      setError(null);
      setSuccessMessage("Your profile has been updated successfully!"); // Show success message
    } catch (err) {
