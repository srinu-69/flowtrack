...
import React, { useState, useEffect } from "react";
// Touch file to trigger hot-reload during dev
console.debug && console.debug("App module loaded");
// touch to trigger reload after API normalization
console.debug && console.debug('App reload touch');
// hot-reload touch for update/delete wiring
import {
  Routes,
  Route,
  NavLink,
...
function Layout() {
  ...
  const styles = {
    ...
    logo: {
      ...
      marginBottom: isMobile ? "24px" : "32px",
      justifyContent:
        sidebarOpen || mobileSidebarOpen ? "flex-start" : "center",
    },
    logoText: {
      fontSize: isMobile ? "22px" : "26px",
      fontWeight: "700",
  ...
  return (
    <div
      ...
      role="application"
      aria-label="Flow Track application"
    >
      {/* Mobile Sidebar Overlay */}
      {isMobile && (
        <div
      ...
      <aside style={styles.sidebar} aria-label="Sidebar navigation">
        ...
        {user ? (
          showFullUserInfo ? (
            ...
            <div style={styles.userIconOnly}>
              <FiUser size={38} style={{ color: "#19a0f7" }} />{" "}
              {/* CHANGED: user icon larger when sidebar collapsed */}
            </div>
          )
        ) : (
          <NavLink to="/login" style={styles.navLink}>
